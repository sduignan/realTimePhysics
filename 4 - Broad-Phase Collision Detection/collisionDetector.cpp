...
void CollisionDetector::broadPhase(std::vector<RigidBody*> bodies, int num) {
	std::vector<RigidBody*> potential_colliders;
	potential_colliders = boundingSpheres(bodies, num);
	obb(potential_colliders);
}

std::vector<RigidBody*> CollisionDetector::boundingSpheres(std::vector<RigidBody*> bodies, int num) {
	...
	for (int i = 0; i < num; i++) {
		...
		if (bodies[i]->colliding) {
			colliders.push_back(bodies[i]);
		}
	}
	return colliders;
}

void CollisionDetector::obb(std::vector<RigidBody*> bodies) {
