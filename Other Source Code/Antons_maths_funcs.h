...
/*------------------HEY ANTON, 3x3 MATRICES ALSO EXIST------------------*/
// returns a 3x3 matrix flipped on the main diagonal
inline mat3 transpose(const mat3& mm) {
	return mat3(
...
inline float determinant(const mat3& mm) {
	return
		mm.m[0] * mm.m[4] * mm.m[8] +
		mm.m[1] * mm.m[5] * mm.m[6] +
		mm.m[2] * mm.m[3] * mm.m[7] -
		mm.m[2] * mm.m[4] * mm.m[6] -
		mm.m[1] * mm.m[3] * mm.m[8] -
		mm.m[0] * mm.m[5] * mm.m[7];
}

inline mat3 inverse(const mat3& mm) {
	...
	return mat3(
		inv_det*(mm.m[4]*mm.m[8] - mm.m[5]*mm.m[7]),
		inv_det*(mm.m[5]*mm.m[6] - mm.m[3]*mm.m[8]),
		inv_det*(mm.m[3]*mm.m[7] - mm.m[4]*mm.m[6]),
		inv_det*(mm.m[2]*mm.m[7] - mm.m[1]*mm.m[8]),
		inv_det*(mm.m[0]*mm.m[8] - mm.m[2]*mm.m[6]),
		inv_det*(mm.m[1]*mm.m[6] - mm.m[0]*mm.m[7]),
		inv_det*(mm.m[1]*mm.m[5] - mm.m[2]*mm.m[4]),
		inv_det*(mm.m[2]*mm.m[3] - mm.m[0]*mm.m[5]),
		inv_det*(mm.m[0]*mm.m[4] - mm.m[1]*mm.m[3])
	);
}

inline mat3 orthonormalise(mat3 mm) {
	//Gram-Schmidt orthogonalisation
	vec3 col_0 = normalise(vec3(mm.m[0], mm.m[1], mm.m[2]));
	vec3 col_1 = vec3(mm.m[3], mm.m[4], mm.m[5]);
	col_1 = normalise(col_1 - col_0*dot(col_1, col_0));
	vec3 col_2 = vec3(mm.m[6], mm.m[7], mm.m[8]);
	col_2 = normalise(col_2 - col_0*dot(col_2, col_0) - col_1*dot(col_2, col_1));
	return mat3(col_0, col_1, col_2);
}
...
3  7 11 15
*/
inline mat4 mat3_to_mat4(mat3 top_left) {
	mat4 res = identity_mat4();
	res.m[0] = top_left.m[0];
	res.m[1] = top_left.m[1];
	res.m[2] = top_left.m[2];
	res.m[4] = top_left.m[3];
	res.m[5] = top_left.m[4];
	res.m[6] = top_left.m[5];
	res.m[8] = top_left.m[6];
	res.m[9] = top_left.m[7];
	res.m[10] = top_left.m[8];
	return res;
}

//This doesn't belong here but I need it.
